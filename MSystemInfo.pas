...
uses
  SysUtils, Windows, Classes,Registry, MMSystem, WinSock, Printers{ DsgnIntf};
...
begin
  ...
  if (GetCPUIDFlags and 16) <> 16 then
    exit;
  result:=DWORD(GetTimeStampHi);
  result:=GetTimeStampLo;
end;
...
begin
  try
    FSystemUpTime:=GetTimeStamp/GetTicksPerSecond(1);
  except
    FSystemUpTime:=0;
  end;
...
begin
  try
    FLastBoot:=Now-(GetTimeStamp/GetTicksPerSecond(1))/(24*3600);
  except
    FLastBoot:=0;
  end;
