...
PROCEDURE MiniMODThreadProc(MiniMOD:TMiniMOD);
BEGIN
 WHILE NOT MiniMOD.ThreadTerminated DO BEGIN
  MiniMOD.Enter;
  MiniMOD.Poll;
  MiniMOD.Leave;
  SLEEP(50);
...
BEGIN
 ...
 OutputBufferSize:=CMiniMODBufferSize;

 WaveFormat.wFormatTag:=WAVE_FORMAT_PCM;
 IF WithStereo THEN BEGIN
  WaveFormat.nChannels:=2;
  OutputBufferSize:=OutputBufferSize SHL 1;
 END ELSE BEGIN
  WaveFormat.nChannels:=1;
 END;
 IF In16Bit THEN BEGIN
...
{$IFDEF FPC}
 ThreadHandle:=CreateThread(NIL,0,@MiniMODThreadProc,SELF,CREATE_SUSPENDED,ThreadID);
{$ELSE}
 ThreadHandle:=BeginThread(NIL,0,@MiniMODThreadProc,SELF,CREATE_SUSPENDED,ThreadID);
{$ENDIF}
...
BEGIN
 ...
 Leave;
 WaitForSingleObject(ThreadHandle,25);
...
PROCEDURE TMiniMOD.Enter;
BEGIN
 EnterCriticalSection(ThreadCriticalSection);
END;

PROCEDURE TMiniMOD.Leave;
BEGIN
 ...
 FOR ChannelNr:=1 TO HowManyChannels DO BEGIN
  IF Channel[ChannelNr].Active THEN BEGIN
   ...
   IF Channel[ChannelNr].TempPeriod>0 THEN BEGIN
    Channel[ChannelNr].Increment:=((Hz DIV Channel[ChannelNr].TempPeriod) SHL CMiniMODPositionShift) DIV Frequency;
   END ELSE BEGIN
...
BEGIN
 ...
 IF RESULT THEN ResumeThread(ThreadHandle);
END;
...
PROCEDURE TMiniMOD.MixBuffer(DestBuffer:POINTER);
VAR Counter:LONGWORD;
    ...
    Buf:PLONGINT;
BEGIN
 FILLCHAR(Buffer,SIZEOF(TMiniMODBuffer),#0);
 ...
 IF Stereo THEN BEGIN
  IF Bit16 THEN BEGIN
   FOR Counter:=1 TO CMiniMODBufferSize DO BEGIN
    WORD(SourcePointer^):=LONGWORD(Buf^ SHR (16-CMiniMODMixerShift));
    INC(LONGWORD(SourcePointer),2);
    INC(Buf);
    WORD(SourcePointer^):=LONGWORD(Buf^ SHR (16-CMiniMODMixerShift));
    INC(LONGWORD(SourcePointer),2);
    INC(Buf);
  ...
  END ELSE BEGIN
   FOR Counter:=1 TO CMiniMODBufferSize DO BEGIN
    BYTE(SourcePointer^):=LONGWORD(Buf^ SHR (24-CMiniMODMixerShift)) XOR $80;
    INC(LONGWORD(SourcePointer));
    INC(Buf);
    BYTE(SourcePointer^):=LONGWORD(Buf^ SHR (24-CMiniMODMixerShift)) XOR $80;
    INC(LONGWORD(SourcePointer));
 ...
 END ELSE BEGIN
  IF Bit16 THEN BEGIN
   FOR Counter:=1 TO CMiniMODBufferSize DO BEGIN
    WORD(SourcePointer^):=LONGWORD(((Buf^+PLONGINT(LONGWORD(LONGWORD(Buf)+4))^) DIV 2) SHR (16-CMiniMODMixerShift));
    INC(LONGWORD(SourcePointer),2);
    INC(Buf,2);
   END;
  END ELSE BEGIN
   FOR Counter:=1 TO CMiniMODBufferSize DO BEGIN
    BYTE(SourcePointer^):=LONGWORD(((Buf^+PLONGINT(LONGWORD(LONGWORD(Buf)+4))^) DIV 2) SHR (24-CMiniMODMixerShift)) XOR $80;
    INC(LONGWORD(SourcePointer));
    INC(Buf,2);
   END;
  END;
